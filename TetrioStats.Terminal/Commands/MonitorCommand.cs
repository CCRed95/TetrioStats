...
namespace TetrioStats.Terminal.Commands
{
		...
		: TerminalCommand<string>
	{
		...
		public override void Execute(string args)
		{
			...
			_trRankingsThresholds = UserRankExtensions.CalculateRankGradeTRThresholdsNew();
			...
			{
				var currentGradeStr = key.GetDescription().ToUpper();
				...
				var rankPercentile = lastThreshold == null
					? key.GetPercentileRange().Maximum
					: key.GetPercentileRange().Maximum - lastThreshold.Value.GetPercentileRange().Maximum;
				...
				XConsole
					.ScopedBold(c1 =>
					{
						c1.ScopedBackground(darkened,
							c2 =>
							{
								c2.Write($"     {key.GetDescription(),-2}    |    ", lightened)
			...
			while (true)
			{
				...
				) //_timeSinceQuery.Elapsed > TimeSpan.FromMinutes(5.1)
				{
					...
					var response = _client.Users.Summaries.FetchLeagueSummaryAsync(userInfo.Data.Username).Result;

					_timeCacheExpires = response.CacheInfo.CachedUntil;


					var _40LinesResponse = _client.Users.Summaries.FetchSprintSummaryAsync(userInfo.Data.Username).Result;
					...
					var user = _coreContext.Users.Single(t => t.TetrioUserID == userInfo.Data.Id);

					var statisticsEntry = new UserStatisticsEntry(
						user,
						response.CacheInfo.CachedAt,
						userInfo.Data.XP,
						response.Data.GamesPlayed.GetValueOrDefault(),
						response.Data.GamesWon.GetValueOrDefault(),
						response.Data.GamePlayTime,
						tlSummary.Data.GamesPlayed.GetValueOrDefault(),
						tlSummary.Data.GamesWon.GetValueOrDefault(),
						tlSummary.Data.TR.GetValueOrDefault(),
						tlSummary.Data.Glicko.GetValueOrDefault(),
						tlSummary.Data.RatingDeviation.GetValueOrDefault(),
						tlSummary.Data.Rank.ToString(),
						tlSummary.Data.APM.GetValueOrDefault(),
						tlSummary.Data.PPS.GetValueOrDefault(),
						tlSummary.Data.VS.GetValueOrDefault(),
						tlSummary.Data.Standing.GetValueOrDefault(),
						tlSummary.Data.LocalStanding.GetValueOrDefault(),
						tlSummary.Data.Percentile.GetValueOrDefault(),
						tlSummary.Data.PercentileRank.ToString());
					...
					var currentRankStr = tlSummary.Data.Rank.Value.GetDescription();
					var nextRankStr = response.Data.NextRank.Value.GetDescription();
					...
					XConsole
						.WriteLine()
						.WriteLine(
							$" {Math.Round(trUntilNextRank)} TR until {nextRankStr} rank (~{Math.Round(trUntilNextRank / 180d, 1)} match wins)",
							Swatch.Yellow);

					XConsole.WriteLine(
							$" {Math.Round(trUntilPreviousRank, 1)} TR loss until losing {currentRankStr} rank (~{Math.Round(trUntilPreviousRank / 180d, 1)} match losses)",
							Swatch.Yellow)
						.WriteLine()
						.WriteLine();
				...
				else
				{
					...
					XConsole
						.Write($" {longestTimeRemaining.Minutes}:{longestTimeRemaining.Seconds:00} remaining", Swatch.Orange)
