...
public static class UserRankExtensions
{
			...
			this UserRank @this)
	{
		...
		if (@this == UserRank.XPlus)
			return (0.0, maxPercentileInRank);

		var nextRankIntegralValue = (int)@this + 1;
		var nextRank = Enum.ToObject(typeof(UserRank), nextRankIntegralValue).As<UserRank>();

		//if (nextRank == UserRank.Z)
		//{
	...
	public static IReadOnlyDictionary<UserRank, DoubleRange> CalculateRankGradeTRThresholdsNew()
	{
		var client = new TetrioApiClient();
		...
		var mapping = leagueRankings.Data.Data.UserRankMapping;
		
		var i = 0;

		foreach (var rankData in mapping)
		{
			var userRankRange = rankData.Key.GetPercentileRange();

			var index = (int)Math.Round(userRankRange.Minimum / 100d * i);
			var upperIndex = (int)Math.Round(userRankRange.Maximum / 100d * i + 1);

			var thresholdTRForRank = mapping.Values.ElementAt(index).TR;
			var upperThresholdTRForRank = mapping.Values.ElementAt(upperIndex).TR;

			results.Add(rankData.Key, (thresholdTRForRank, upperThresholdTRForRank));
			
			i++;
		}

		//results.Add(UserRank.XPlus, (thresholdTRForRank, upperThresholdTRForRank));
		//foreach (var userRank in userRanks)
		//{
		//	var userRankRange = userRank.GetPercentileRange();
...
public static class EnumExtensions
{
	...
	[CanBeNull]
	public static TEnum? TryParseFromDescription<TEnum>(string @this)
